...
The flowchart below illustrates the overall flow taken by an user when working with the Contextual Awareness feature. +

image::awarenessFlow.png[width="800"]

The diagrams shows that there are 2 functions provided as part of the Awareness feature. 
One is a real-time service that updates the User Interface **as the user types (without pressing `Enter`) **
The other attempts to add a ResumeEntry, and is triggered only when the user presses `Enter`.

Both services required the following steps to be executed.
...
We will first examine these 2 steps. 
Information about implementation of the real-time service is given below.

==== Guessing an Event name, and Matching it with a ResumeEntry (based on user provided `expression`)
The Awareness class exposes a `getPossibleEventName` method that accepts a `expression` and returns a `possibleEventName`. +
The `getPossibleEventName` corresponds to Step 1 mentioned in <<Overview, overview>>. +
...
Thus, the `possibleEventName` serves as a unique `key` to identify a `ResumeEntry`.
The `getContextualEntry` method corresponds to Step 2 mentioned in <<Overview, overview>>.

The breakdown given above is illustrated in the following sequence diagram.
